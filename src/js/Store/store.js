...
const parseLastEvents = function(teamId, lastEvents) {
  return lastEvents.reduce((events, event) => {
    ...
    if (
      ...
      event.scores[0].hasOwnProperty('homeScore')
    ) {
      ...
      if (event.scores[0].homeScore === event.scores[0].awayScore) {
        ...
        event.homeParticipant.participantId === teamId &&
        event.scores[0].awayScore > event.scores[0].homeScore
      ) {
        result = 'lose'
      } else if (
        event.awayParticipant.participantId === teamId &&
...
const getParticipants = function(eventId) {
  ...
  return statisticsModule.getTeamPerformanceStatistics(eventId).then(data => {
    return [
      {
        id: data.homeParticipant.participantId,
        name: data.homeParticipant.participantName,
        lastEvents: parseLastEvents(
          data.homeParticipant.participantId,
          data.homeParticipant.lastEvents
        ),
      },
      {
        ...
        lastEvents: parseLastEvents(
          data.awayParticipant.participantId,
          data.awayParticipant.lastEvents
        ),
      },
    ]
  })
